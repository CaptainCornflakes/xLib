function Img = XYZ2CIECAM02c(Img,WP, La,Yb,Surround)
%Lab2XYZ Convert linear XYZ to CIECAM12
...
MHPE  = [0.38971 0.68898 -0.07868;...   % Eqn.(12)
        -0.22981 1.18340  0.04641;...
         0.0     0.0      1.0    ];
%% Transfer directly to Hunt-Pointer-Estevez LMS-Cone-Space
...
LMSs = ([(1-2*c1), c1, c1;
        c2, (1-2*c2), c2;...
        c3, c3, (1-2*c3)]*(MHPE*(Img')))';
WP_LMSs = ([(1-2*c1), c1, c1;
        c2, (1-2*c2), c2;...
        c3, c3, (1-2*c3)]*(MHPE*(WP')))';

if min(min(LMSs)) < 0
    warning(['LMS Values below zero: ' num2str(min(LMSs)) ' Limiting to ZERO!'])
     LMSs(LMSs<0)=0;
end
...
%% No need to Transfer back to Hunt-Pointer-Estevez LMS-Cone-Space because we are already there
%% Introduce Nonlinearity:
LMS_p = (400*(LMSa*Fl/100).^0.42)./(27.13 + (LMSa*Fl/100).^0.42) + 0.1;
WP_LMS_p = (400*(WP_LMSa*Fl/100).^0.42)./(27.13 + (WP_LMSa*Fl/100).^0.42) + 0.1;
...
if isa(Img,'xBase')
    Img = Img.setHistory('Converted from XYZ to CIECAM12');
end
