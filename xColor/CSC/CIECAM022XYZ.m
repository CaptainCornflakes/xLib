function Img = CIECAM022XYZ(Img,WP, La,Yb,Surround)
...
% Img = XYZ2CIECAM02(ChkImg,WP,La,Yb,Surround);
...
%% Step 3.1: Calculate  AW
iWP_LMSs = (CAT02*(WP'))';                                      % Transfer to shapened CAT02 LMS space
iWP_LMSsa = ([WP(2)*D/iWP_LMSs(1) + 1 - D, 0, 0;...             % Adapt
             0, WP(2)*D/iWP_LMSs(2) + 1 - D, 0;...
             0, 0, WP(2)*D/iWP_LMSs(3) + 1 - D] * iWP_LMSs')';  
iLMS_WP = (MHPE/CAT02*iWP_LMSsa')';                             % Transfer back to Hunt-Pointer-Estevez LMS-Cone-Space
iWP_LMS_p = (400*(iLMS_WP*Fl/100).^0.42)./(27.13 + (iLMS_WP*Fl/100).^0.42) + 0.1; % Introduce Nonlinearity
...
iLMS = 100/Fl * (((27.13 * abs(iLMS_p - 0.1)) ./ (400 - abs(iLMS_p - 0.1))) .^ (1 / 0.42));
...
iLMS((iLMS_p - 0.1) < 0 & (iLMS > 0)) = -iLMS((iLMS_p - 0.1) < 0 & (iLMS > 0));

%% Transfer from Hunt-Pointer-Estevez LMS-Cone-Space to CAT02 CONESpace
iLMSsa = (CAT02/MHPE*iLMS')';

%% Revert Adaption
iLMSs = ([WP(2)*D/iWP_LMSs(1) + 1 - D, 0, 0;...
          0, WP(2)*D/iWP_LMSs(2) + 1 - D, 0;...
          0, 0, WP(2)*D/iWP_LMSs(3) + 1 - D] \ iLMSsa')';

%% Transfer from shapened CAT02 LMS space to XYZ
Img = (CAT02\(iLMSs'))';
...
if isa(Img,'xBase')
    Img = Img.setHistory('Converted from CIECAM02 to XYZ');
end
