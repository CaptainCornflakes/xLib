function IPTPQ = XYZ2IPTPQflex(XYZ_D65,XYZWhitepoint,param)
...
LMS = (param{1}*(XYZ65_2_LMS*XYZ_D65'))';
...
% % Compress based on C
% comp = param(7);
% squaredSat = IPTPQ(:,2).^2+IPTPQ(:,3).^2;
% IPTPQ(:,2) = IPTPQ(:,2).*(1-comp.*squaredSat);
% IPTPQ(:,3) = IPTPQ(:,3).*(1-comp.*squaredSat);
